...
1. `on-message-received` (`before_agent_start`) carries the turn-start harness-first search/read-debt reminder, map-first protocol, and interactive grammar.
2. `on-context.sh` (Pi/OMP `context` event, fired after file-touching tool results) re-injects a compact `<system-reminder>` that **leads with**: relevant-record search (code/tests read ≠ record-grounding; `lazy map` + governing `.lazy-harness/<layer>` record before action, §2.1/§2.5) and turn-end capture (decisions/corrections/learnings → record, §2.4), plus the interactive grammar (record↔code conflict→ask, option gate, requirements-first; §0/§2.3). `index.ts` wires the `context` handler with `FILE_OP_TOOLS`/`pendingRegroundByRoot`/`regroundBodyByRoot` — one inject per new file-op batch, reset per turn, fail-open.
...
- Agents that touch files get the relevant-record-search and turn-end-capture mandates re-surfaced mid-turn, closing the "read code → act / end without capturing" drift — matching jcode's mechanism.
- **Honest limitation — organic, not hard.** jcode's `load_harness_dir` force-injected the grammar into the system prompt every session (config-guaranteed). OMP/Pi have no equivalent; the re-injection is an advisory `system-reminder` the agent must heed. A true hard guarantee needs an OMP/Pi runtime feature to force the full grammar into every session's prompt — out of extension scope.
