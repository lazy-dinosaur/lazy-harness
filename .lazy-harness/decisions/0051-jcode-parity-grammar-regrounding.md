...
1. `on-message-received` (`before_agent_start`) carries the turn-start harness-first search/read-debt reminder, map-first protocol, interactive grammar, and a **review/gap-analysis driver**: on a "what is missing/undecided/incomplete"/audit request, enumerate `lazy policy list` + `lazy capability list` and map the governing records BEFORE reading code (AGENTS §1/§2.1).
