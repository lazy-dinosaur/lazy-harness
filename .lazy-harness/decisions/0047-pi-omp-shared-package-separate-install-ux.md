...
- `.lazy-harness/scripts/pi-package.ts` becomes runtime-aware and dispatches Pi and OMP command arrays from the same source wrapper.
- `.lazy-harness/bin/lazy` owns distinct `pi` and `omp` subcommands.
- Regression coverage must protect both Pi and OMP wrapper dry-runs and the explicit OMP manifest.
