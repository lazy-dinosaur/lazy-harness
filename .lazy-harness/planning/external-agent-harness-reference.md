...
| TimSquad (`sonature-lab/timsquad`, npm `timsquad` v3.8.0, MIT, EricSon/sonature-lab; Claude Code runtime) | "Vibe Development Framework": SSOT-based documentation (level-scaled required doc sets 5/11/14 by project Level 1/2/3) + 7 Controller-delegated agent roles (architect/developer/qa/security/dba/designer/librarian) + retrospective learning; 5-Layer Enforcement Architecture with Layer 1 Hook Gate (100% enforced) via Claude Code PreToolUse hooks + Capability Token (system-level blocking, fail-closed); 13 hook gates incl safe-guard, tdd-guard, stale-guard (SSOT 7-day freshness), completion-guard (test+TDD+SSOT), build-gate; 37 `tsq-*` skills incl tsq-ddd/tsq-bdd/tsq-tdd/tsq-spec; model routing (haiku↔opus); Wave Dispatch parallel tasks; observer-only daemon (decisions.jsonl decision log, SSOT drift detection, session metrics) | hard fail-closed enforcement of the SAME layer methodology (SSOT/DDD/BDD/TDD/ADR), Capability Token per-task write isolation, SSOT drift/staleness detection, level-scaled required doc sets, zero-token observer daemon telemetry, productization (npm package, templates, public docs) | closest METHODOLOGICAL TWIN of lazy-harness, on the Claude Code runtime (not Pi → not installable here, no tool conflict; study only); proves lazy-harness's own SSOT/DDD/TDD methodology CAN be hard-gate enforced fail-closed — the key input for revisiting ADR 0016's advisory-only dev-time stance |
...
- 2026-07-04 source-level correction: the v0.1.4 thin-wrapper description above covers only the Pi integration package; upstream `a5c-ai/babysitter` is a 17,245-file monorepo (`babysitter` v6.0.2, 8 packages) with a full SDK — see "Source-level deep dive" below.
...
### lazy-harness vs TimSquad contrast

- Closest methodological twin yet: TimSquad independently ships lazy-harness's exact layer set (SSOT docs + tsq-ddd/bdd/tdd/spec skills + architect-driven ADR) as a public npm product — but on the Claude Code runtime, not Pi.
- Same methodology, opposite enforcement: lazy-harness = advisory/organic dev-time hooks, hard gate only at git pre-commit/push (ADR 0016); TimSquad = fail-closed hard gates at dev time (PreToolUse Hook Gate + Capability Token: tdd-guard, completion-guard, build-gate, stale-guard, safe-guard).
- Where TimSquad is ahead (study targets): hard fail-closed enforcement of the shared methodology; Capability Token per-task write isolation; SSOT drift detection (7-day stale); level-scaled required doc sets (5/11/14); observer-only daemon (decision log + metrics without token cost); productization/packaging (see `.lazy-harness/plans/timsquad-level-product-maturity-gap-closure.md`).
- Where lazy-harness is ahead: depth of layered institutional memory (knowledge graph + implementation maps + Rule digests), user-correction convergence (ADR 0032), methodology pluralism vs TimSquad's TDD-leaning gates (ADR 0024), map-first evidence retrieval, decision permanence via ADR supersede chains.
- Headline learning: TimSquad proves the shared SSOT/DDD/TDD methodology can be hard-gate enforced fail-closed — directly addressing lazy-harness's #1 recorded weakness (enforcement advisory until git boundaries). The open question is a narrow revisit of ADR 0016, aligning with the Babysitter verdict: determinism only on already-hard gates, not wholesale conversion of advisory hooks.
- What to NOT adopt: Claude Code runtime dependency; converting all dev-time advisory hooks into rigid blocks. Verdict: non-adopt as a runtime (different runtime, not installable in this Pi setup); study only.
- History: `.lazy-harness/plans/timsquad-level-product-maturity-gap-closure.md` (2026-05-14 product-maturity gap plan), `.lazy-harness/retrospective/audits/2026-05-12-lazy-vs-timsquad-comparison.md` (comparison audit), ADR 0001/0016/0024 contrast mentions, `candidates.jsonl` 2026-06-30 deep evaluation.

### Source-level deep dive — 2026-07-04 (local clones)

- Method: cloned `sonature-lab/timsquad`, `Gentleman-Programming/gentle-pi`, `a5c-ai/babysitter` (depth 1) to `/tmp/harness-analysis/` and read guard scripts / extensions / SDK source directly. Framing axes (user-set): (A) accuracy improves through real use (learning loop), (B) project knowledge catches errors.
- TimSquad (502 files, Claude Code): the ONLY harness of the three with a closed learning loop — `tsq-retro`: L1/L2/L3 feedback classification → `feedback.jsonl` → KPT retro over logs/metrics → failure patterns (FP, 3+ repeats) / success patterns registered → `improve` mode patches SKILL.md rules / templates / `lessons.md` with mandatory user approval, effect measured next retro. Hook pipeline verified in source: safe-guard / phase-guard / capability-token / change-scope (PreToolUse, fail-closed) + completion-guard & build-gate (Stop **deny** — untested or unbuilt work cannot be declared complete) + pre-compact / context-restore; daemon appends `session-notes.jsonl` snapshots re-injected each Stop. Caveats found in source: stale-guard is warn-only (systemMessage) despite fail-closed docs; tdd-guard checks test-file existence only (formally bypassable); knowledge representation is flat SSOT docs (no graph / implementation map); hard Claude Code lock-in (bash+jq hook protocol).
- gentle-pi (103 source files, ~6.6k TS lines, Pi-native): unique spec-vs-spec error detection — `lib/openspec-guardrails.ts` detects active-change domain collisions and destructive deltas (REMOVED / large-MODIFIED requirements): a defect class lazy-harness does not yet detect between records. `lib/review-triggers.ts` (pure, unit-testable): pre-commit/pre-push/pre-pr/post-sdd-phase × pathGlobs/minDiffLines/phase → 4R review agents, `advisory|strong` modes — philosophically compatible with ADR 0016. Runtime guard: non-overridable hard-deny patterns + per-command allow/confirm/block. Learning loop only partial: Engram memory is an external optional Go binary (preflight just warns); skill-improver/skill-creator improve skills on request (audit-only default); no failure-pattern accumulation.
- Babysitter (upstream monorepo, 17,245 files, v6.0.2): core = event-sourced journal (`storage/journal.ts`, append-only, atomic, lock, ULID), HITL breakpoints with CRYPTOGRAPHIC answer verification (`breakpoints/proven-verification.ts` — signature/fingerprint check makes forged approvals detectable), policy engine, HarnessAdapter layer (`harness/BaseAdapter.ts` + registry/discovery — a working implementation of design idea #1 / R5), ~30 methodologies shipped as executable JS processes (DDD, BDD specification-by-example, event-storming, spec-kit, BMAD, cleanroom, XP…). NO learning loop by design (0 grep hits for retrospective/lessons/memory accumulation in SDK src): accuracy comes from determinism + replay, not accumulation — the exact inverse of the user's axis (A).
- Axis summary: (A) learning loop — TimSquad strong / gentle-pi partial / Babysitter absent / lazy-harness strong on fact-and-correction accumulation but MISSING an automatic failure-pattern→rule-reinforcement cycle. (B) knowledge-driven error catching — TimSquad Stop-deny + phase/scope gates / gentle-pi spec-collision + conditional review / Babysitter deterministic gates + signed breakpoints / lazy-harness record-lint + self-test + advisory hooks (no dev-time realtime blocking).
- Adoption shortlist (captured as candidates, NOT approved): (1) TimSquad-style retro loop layered on candidates.jsonl/policies.json (feedback → KPT → 3-repeat pattern promotes to record/policy); (2) Stop-deny completion/build gate as a narrow fail-closed gate (ADR 0016-compatible per all three contrasts); (3) record-vs-record contradiction detection as record-lint's next step (gentle-pi guardrails class, 58 SDD corpus); (4) conditional review triggers (path/diff/phase × advisory|strong) aligned with capability levels (ADR 0040); (5) Babysitter journal/adapter strictly as reference implementation for replay-eval and R5.
- Clones retained at `/tmp/harness-analysis/` (transient; re-clone as needed).
...
- Primary files:
  ...
  - third-pass external references: TimSquad (`sonature-lab/timsquad`, Claude Code methodology twin; history in `.lazy-harness/plans/timsquad-level-product-maturity-gap-closure.md`).
- Key symbols:
...
- Machine index:
  ...
  - third-pass graph ids: `kg_external_agent_harness_reference_timsquad_governance`
