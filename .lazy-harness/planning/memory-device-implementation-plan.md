...
3. Stage order per host: ① record-quality issues (digest format/broken refs — lazy-record-quality) → ② memory backfill (surface terms in the host's own domain vocabulary + reachability repairs — lazy-memory-backfill) → ③ graph cleanup (legacy-shape/removed-framework/missing-path rows — lazy-graph-migrate, reason-based) → ④ host's own gate-promotion decision after reaching advisory 0 (mirror of the framework's post-W8 gate).
4. Graph-cleanup session-open message (verbatim template, added 2026-07-05; jcode guidance corrected 2026-07-05): "lazy update 먼저 돌려서 최신 graph detector 받고, lazy-graph-migrate 스킬로 우리 그래프 정리하자. 각 행은 '오래됨'이 아니라 실제 이유로 판단해줘 — 사실이 아직 참이면 KEEP, framework 능력이 ADR 0050으로 제거돼 obsolete면 SUPERSEDE(이유 명시), 없는 경로(missing-path)는 FIX/ADD/SUPERSEDE 케이스별로. 배치(5~10)로 제안해서 내 승인 받고 append-only로만, host 그래프 직접 수정 금지. record 본문 jcode 언급은 어느 스킬도 자동 안 하니 — 역사 서술은 KEEP, 진짜 stale 현재형 주장만 수동 케이스별로 (lazy-record-quality 아님). feature-navigation.xml 참조 행은 현행이니 KEEP(supersede 금지)." — note: rows that are still-true framework-internal facts in legacy shape are KEEP (do NOT bulk-convert; downstream-host detector-conflict decision); active rows referencing ADR-0050-removed capabilities are the SUPERSEDE-with-reason set; feature-navigation.xml is current (Project Map V2 uses it) so its rows are KEEP.
5. Progress persists in the host's plan/records between sessions (same batch-log pattern as W8); graph edits are append-only supersession, never host-graph deletion/rewrite.
