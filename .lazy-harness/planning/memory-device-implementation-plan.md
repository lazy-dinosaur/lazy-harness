...
4. Graph-cleanup session-open message (verbatim template, added 2026-07-05): "lazy update 먼저 돌려서 최신 graph detector 받고, lazy-graph-migrate 스킬로 우리 그래프 정리하자. 각 행은 '오래됨'이 아니라 실제 이유로 판단해줘 — 사실이 아직 참이면 KEEP, framework 능력이 ADR 0050으로 제거돼 obsolete면 SUPERSEDE(이유 명시), 없는 경로(missing-path)는 FIX/ADD/SUPERSEDE 케이스별로. 배치(5~10)로 제안해서 내 승인 받고 append-only로만, host 그래프 직접 수정 금지. record 본문 jcode 언급은 범위 밖이니 lazy-record-quality로 따로." — note: rows that are still-true framework-internal facts in legacy shape are KEEP (do NOT bulk-convert; medivance detector-conflict decision); active rows referencing ADR-0050-removed capabilities are the SUPERSEDE-with-reason set.
