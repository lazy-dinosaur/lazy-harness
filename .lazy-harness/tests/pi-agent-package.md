...
| `lazy_agent_activation_prompts` | Run `lazy agent activate` twice against a temp initialized git repo with isolated Jcode home, then trigger a Jcode local-config conflict | `.pi/APPEND_SYSTEM.md` and `.omp/APPEND_SYSTEM.md` contain managed pointer prompts, `.pi/settings.json` contains project-relative Claude/Codex/Agents skill paths plus `enableSkillCommands: true`, exact-root Jcode trust/local transport is idempotent, and trust failure rolls Pi/OMP/exclude state back byte-exactly |
| `lazy_init_sync_unified_activation` | Initialize a temporary Git host, sync while trusted, explicitly untrust it, and sync again | Init activates Pi/OMP/Jcode for the selected target; trusted sync checks/repairs activation; untrusted sync reports the explicit activation command without restoring trust |
