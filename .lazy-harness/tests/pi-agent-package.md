...
- Must:
  ...
  - surface a visible per-turn `lazy-harness armed` marker with runtime marker/root/read-debt/tool-guard status, and block lazy-root action tools with `read-debt not armed` when the turn did not arm instead of silently fail-opening
- Must not:
...
| `pi_extension_before_agent_start_bridge` | Inspect extension source | Source contains `before_agent_start`, calls `on-message-received.sh`, injects `REMINDER. Harness-first search/read debt before response.` fallback, and emits a visible `lazy-harness armed` marker with runtime marker/root/read-debt/tool-guard status |
