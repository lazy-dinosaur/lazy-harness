...
- Must:
  ...
  - emit overview/concrete-map-node/missing-prerequisite cues as evidence cues
...
- `candidates`: record/source/test/graph candidates and unresolved terms, including top-level Related layer record paths surfaced from matched records
- `commands`: overview, concrete map-node commands copied from structural candidates, and missing-prerequisite cue
...
5. For `gap` or `partial`, LLM/searcher follows concrete map-node candidates; if none exist, it uses an option gate or missing-prerequisite statement instead of keyword fallback.
