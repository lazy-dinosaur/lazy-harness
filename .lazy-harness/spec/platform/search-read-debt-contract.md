...
→ inject compact harness-first reminder
→ LLM/searcher performs root-bound record/source/test search/read
...
- Primary files:
  - `.lazy-harness/hooks/lifecycle/on-message-received.sh` — writes static `message.received.search-read-debt` rows and injects the compact reminder.
