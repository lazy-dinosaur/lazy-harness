...
Before asking the user to restate known facts, the Project Profile flow should inspect existing non-harness project documents and propose which facts should be internalized into `.lazy-harness` records. External documents remain source evidence, but durable AI-operational knowledge belongs in DDD/SDD/BDD/TDD/ADR/SSOT/project records.
...
- existing project documents outside `.lazy-harness` that should be assimilated into records, such as `README.md`, `docs/**`, architecture notes, product briefs, onboarding docs, legacy planning docs, and design-system notes
...
- `.lazy-harness/project/document-intake.xml` or equivalent intake ledger mapping external documents to generated/updated records
...
The Project Profile skill should have an intake stage:

```text
scan existing docs
→ summarize source evidence
→ classify each durable fact into DDD/SDD/BDD/TDD/ADR/SSOT/project profile
→ ask option gates for ambiguous or architectural decisions
→ write/update records with source references
→ keep an intake ledger so future agents know what was assimilated and what was intentionally skipped
```

Rules:

- Do not treat external docs as canonical once a `.lazy-harness` record exists. The record becomes the operational source for agents.
- Do not silently rewrite uncertain architecture from old docs. Present options when docs conflict or are stale.
...
- Keep provenance: every internalized fact should link to the original document path and section when available.
- Prefer root-bound host docs only. Do not import sibling repository knowledge unless the user explicitly points to it.
...
- `.lazy-harness/scripts/knowledge-intake.ts`
  - Existing Stage 1 detector for knowledge candidates from text; it never writes records and can be reused by future document-intake tooling.
