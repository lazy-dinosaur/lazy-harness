...
- Must:
  ...
  - support `--migration-plan` (2026-07-05, user-approved): read-only detection only — SURFACE candidates (legacy-shape rows, references to ADR-0050-removed framework files, and ACTIVE non-retired rows with a missing host path that is NOT a removed-framework file → `missing-path-row`) and report record-BODY jcode mentions (DETECTION-ONLY; fixed by MANUAL per-case record-body review, NOT `lazy-record-quality` whose scope is Rule digest + broken refs — no skill auto-edits record bodies). Proposals are LLM-VERDICT candidates, not directives: the s/p/o mapping is a HINT to verify; per-row verdict is KEEP / CONVERT-if-fact-holds / SUPERSEDE-by-real-reason for legacy/removed, and FIX / ADD / SUPERSEDE for `missing-path-row` (never by age/shape) — decided by the `lazy-graph-migrate` guided skill against source, batch + user approval, append+supersede. Proposals never count toward `--fail-on-issues`
- Must not:
...
- `--migration-plan`: append a read-only `migrationPlan` section (JSON) / `## Migration plan` section (md) with counts `legacySchemaRows`, `removedFrameworkRefs`, `recordJcodeMentions` (detection-only record-BODY jcode mentions for MANUAL per-case body review — no skill auto-edits bodies), and `missingPathRows` (ACTIVE non-retired rows whose host path is missing and is not a removed-framework file — FIX/ADD/SUPERSEDE verdict). Still read-only: the CLI never repairs/rewrites/supersedes rows or edits record bodies; the REMOVED_FRAMEWORK_FILES list mirrors `lazy-sync.ts` KNOWN_REMOVED_MANAGED_FILES (co-change both together). Summary also carries `supersededTrails` (legitimate 1-active+superseded id groups).
