...
- Must:
  ...
  - surface a short visible per-turn armed marker (`lazy-harness armed`, runtime marker, root, `read-debt=armed|NOT-ARMED`, `tool-guard=ready`) so users can distinguish extension-loaded/armed turns from stale or unloaded sessions; if a lazy-root action tool runs while the turn was not armed, block with a read-debt-not-armed reason instead of silently fail-opening
- Must not:
...
3. handle `before_agent_start` by invoking `.lazy-harness/hooks/lifecycle/on-message-received.sh` through stdin JSON and appending the returned reminder body — which carries the harness interactive grammar (record↔code conflict, option gate, requirements-first; AGENTS §0/§2.3/§2.5) plus the search/read-debt protocol — to the system prompt, while also injecting a short visible `lazy-harness armed` marker for the turn (`root`, runtime marker, `read-debt=armed|NOT-ARMED`, `tool-guard=ready`),
...
5. return `{ block: true, reason }` when the lazy hook emits a deny reason; additionally, if a lazy root is detected but the active turn has no armed read-debt packet and an action tool is about to run, block with a `read-debt not armed` diagnostic rather than silently allowing action,
