...
- Must:
  ...
  - surface pending host record migration at turn-start (user-approved resume-surfacing decision, 2026-07-05): the `on-message-received.sh` reminder appends a deterministic `Host record migration PENDING` line via `helpers/host_migration_state.py` (bounded fail-open `lazy record-lint --format=json` probe, timeout < extension hook budget) when issues/advisories > 0, pointing at the guided `lazy-record-quality`/`lazy-memory-backfill` resume path — closes the gap where `[Next steps]` stdout was visible only to whoever ran `lazy update` and host agents never saw pending backfill state (retro `fb-mr7g01i9-ui`). Host-state-derived only, never user-text classification; never rewrites records
- Must not:
...
5. return `{ block: true, reason }` when the lazy hook emits a deny reason; additionally, if a lazy root is detected but the active turn has no armed read-debt packet and an action tool is about to run, block with a `read-debt not armed` diagnostic that names the status rather than silently allowing action,
