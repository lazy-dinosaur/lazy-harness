...
- Must:
  ...
  - keep the harness interactive grammar (record↔code conflict→ask, option gate, requirements-first) live across the turn: carry it in the `before_agent_start` reminder and re-inject via the `context` event at most once per turn after the first successful file-operation batch. Parallel or sequential file operations before that callback collapse into the same batch; later file operations in the same turn must not restart the discovery loop. A fresh human turn or explicit mid-turn steer resets the boundary. The `context` re-grounding (`on-context.sh`, fed the turn's `recent_tool_calls`) runs `lazy map` on the touched paths and injects the ACTUAL matching `.lazy-harness/<layer>` record refs + `lazy policy list` operating policies — jcode "instructions relevant to files just read/edited — read and follow" parity: SURFACE the records, do not merely tell the agent to look — plus the relevant-record search (§2.1/§2.5) and turn-end capture (§2.4) mandates. If `on-context.sh` cannot produce that real body, the adapter must fail open silently instead of injecting a generic fallback reminder, because a generic reminder can loop without surfacing actionable relevant records. The `before_agent_start` reminder also carries a review/gap-analysis driver: enumerate policies (`lazy policy list`/`lazy capability list`) + governing records BEFORE reading code (AGENTS §1/§2.1)
  - surface the deterministic operating-rule/capability catalog (shared `helpers/operating_rule_catalog.py`; `lazy capability list` + `lazy policy list`) at the `before_agent_start` turn-start reminder and the single bounded `context` mid-turn re-grounding so stored project rules are visible before action (R3, ADR 0048; jcode full-grammar parity). The catalog is discovery-only: agents must not resolve every listed intent or chain resolver calls. Resolve only the single immediate matching intent when a rule-governed action actually needs it, and do not rerun resolution when the same reminder already contains resolved source-work guidance. The catalog remains user-text-agnostic, advisory/visible, and never a `lazy find --purpose` query backend. Catalog subcommands must use a bounded timeout shorter than the extension hook timeout and fail open, so slow enumeration does not prevent read-debt arming.
  - re-assert proactive record-first once at the first file-operation boundary of each turn (not after every micro-edit/search): the `context` re-grounding (`on-context.sh`) carries a "Map-first BEFORE reading/editing more" push so the once-per-session turn-start protocol does not fade on long turns without fragmenting one coherent edit loop.
  - when touched paths include source code, derive exact source-work intents from mechanical file-tool labels and surface the canonical `lazy capability resolve` / `lazy policy resolve` matches from this host (ids, source records, summaries, actions). The framework Code Organization Profile is a recommend-level baseline rather than a hardcoded only profile; host-project additions must be preserved and surfaced. This transport remains user-text-agnostic and may not infer system architecture, split by line count, warn, block, or rewrite untouched source
  - re-ground and RE-ARM MID-TURN steered user input (jcode delivery parity): Pi's default Enter steers mid-turn and skips `before_agent_start`, so a steered instruction would otherwise inherit the previous topic's read-debt evidence and lose the §2.1 record-search push. For every non-extension, non-empty `event.streamingBehavior === "steer"`, the `input` handler must (1) advance a root-scoped evidence epoch, (2) clear prior recent-tool evidence, (3) transform the text with a compact steer re-ground reminder that fresh post-steer map/read evidence is required and prior approvals may be stale per ADR 0038, and (4) force the next `context` call to re-inject the harness re-grounding body. Every allowed `tool_call` is tagged with its start epoch; a `tool_result` from an older epoch must not repopulate evidence after the steer. This is generic transport enforcement: no steer-text classification, command allowlist, or semantic debt row. Empty and extension-injected inputs remain exempt (ADR 0051 2026-07-03 amendment; fresh-evidence hardening user-approved 2026-07-08).
  - project the jcode-shape payload the `on-response-completed.sh` helpers expect: every current-turn `recent_tool_calls` entry carries string `args_preview`, `edit_target`, `evidence_epoch`, and `is_error`; `agent_end` carries `assistant_response` + `last_user_message` from `event.messages`. Normal turn starts advance the root evidence epoch, and `agent_end` must filter out older epochs. This preserves structural failed-call evidence without letting stale calls satisfy or trigger lifecycle helpers
...
7. handle `tool_result` by retaining bounded root-scoped history for later guards, tagging each accepted result with its current evidence epoch and error state, and flagging the root for mid-turn re-grounding when a file-touching tool (`read`/`edit`/`write`/`grep`/`find`/`ls`) succeeds. Results whose call began in an older normal-turn/steer epoch are ignored.
...
10. handle `context` (fired before each LLM call) by injecting a compact re-grounding reminder message into `messages` — sourced from `.lazy-harness/hooks/lifecycle/on-context.sh` — at most once per normal turn after the first successful file-touching operation batch. Parallel or sequential file operations that complete before the first callback collapse into that batch; after a body is successfully cached/injected, later `read`/`edit`/`write`/`grep`/`find`/`ls` results in the same turn must not set another pending re-ground. `before_agent_start` and an explicit mid-turn steer clear the cached body and allow one fresh injection for the new evidence epoch. Missing, failed, empty, or malformed context hooks fail open for the current callback but keep the pending flag so the next context boundary can retry; pending is cleared only after a valid reminder body exists. For source paths, `on-context.sh` mechanically maps recent file-tool labels to exact source-work intents and surfaces canonical host capability/policy resolver matches. That resolved guidance is authoritative for the reminder: the agent must not rerun the same resolver or chain all catalog intents. Neither layer may classify user text or invent policy semantics. The injected `context` message is a `UserMessage` (`role`/`content`/`timestamp`) wrapping the body in a `<system-reminder>` tag,
...
Project activation is separate from package installation. An activated project is identified by `.lazy-harness/bin/lazy`; the globally loaded extension must no-op when `ctx.cwd` cannot resolve that file. `lazy agent activate --target <repo>` transactionally creates project-local `.pi/APPEND_SYSTEM.md` and `.omp/APPEND_SYSTEM.md` pointer prompts, merges `.pi/settings.json` with project-relative skill paths (`../.claude/skills`, `../.codex/skills`, `../.agents/skills`) and `enableSkillCommands: true`, adds `.pi/` / `.omp/` to that repo's `.git/info/exclude`, and delegates global Jcode hook repair plus exact-root trust/local transport to the canonical Jcode package command using the sync marker's stable framework source. A Jcode install/trust conflict rolls back the Pi/OMP file and exclude changes from that activation. `lazy init` activates the explicitly selected new project by default; `--skip-agent-activation` retains framework-only installation. `lazy sync` repairs activation only for already trusted roots, publishes its version marker only after repair succeeds, and never creates Jcode trust.
...
- `packages/lazy-harness-pi/extensions/lazy-harness/index.ts` — event bridge implementation plus `/lazy-check`, `/lazy-validate`, and explicit fresh/full `/lazy-test` command surfaces.
  ...
  - the `context` handler + `pendingRegroundByRoot`/`regroundBodyByRoot`/`FILE_OP_TOOLS` state collapse successful file operations into one re-grounding injection per turn; failed/empty hooks keep pending state for the next context retry, a cached body suppresses later same-turn retriggers, and `before_agent_start`/explicit steer boundaries reset the cache.
  - `evidenceEpochByRoot` / `toolCallEpochsByRoot` plus `advanceEvidenceEpoch` / `rearmEvidenceAfterSteer` / `toolResultBelongsToCurrentEvidenceEpoch` create normal-turn and steer boundaries, reject late older results, and let `agent_end` project only the active epoch without classifying user text.
  ...
  - `agentEndTracePath` / `writeAgentEndTrace` provide the opt-in, content-free `agent_end` diagnostic and route it through the canonical runtime root without changing hook or continuation semantics.
...
- `.lazy-harness/hooks/lifecycle/on-context.sh` — compact mid-turn re-grounding body for the `context` event; derives exact source-work intents from file-tool labels and requests host-resolved advisory guidance only for source paths.
- `.lazy-harness/hooks/lifecycle/helpers/operating_rule_catalog.py` — bounded operating-rule/capability catalog plus canonical resolver-result rendering for host-specific source guidance; all subprocess calls share a timeout window and fail open.
...
- `.lazy-harness/scripts/self-test.py#check_code_organization_profile` — source/non-source context behavior, recommend-only baseline, and minimal temp-host source-policy adaptation validation.
  ...
  - Fake runtime `agent_end` smoke covers current-turn-only tool projection (including failed current calls and late/previous-turn exclusion), jcode-shape payload fields, trace default-off behavior, runtime-root placement, no-raw-content fingerprints, and unchanged `followUp` advisory delivery.
- Machine index:
