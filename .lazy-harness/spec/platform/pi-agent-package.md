...
- Must:
  ...
  - force-load the FULL `.lazy-harness/AGENTS.md` grammar (§0–§2.5) into the system prompt at `before_agent_start`, once per session, deduped by the `Lazy-Harness AI` title marker and fail-open — OMP/Pi otherwise only load a compact activation pointer, not the grammar body
