...
- Implementation hints:
  - Files: `.lazy-harness/schemas/context-tier-manifest.schema.json`, `.lazy-harness/project/context-tiers.yaml`, `.lazy-harness/fixtures/context-tier/context-tier-manifest.sample.json`, `.lazy-harness/scripts/self-test.py`
...
```text
Which compact records/files are generally useful for a class of work before direct search narrows the task?
```
...
| `optional` | Helpful background, never required by tier membership alone. | Discovery, reviewer hints, or later retrieval cues. |
...
The logical schema is defined in `.lazy-harness/schemas/context-tier-manifest.schema.json` and shown as JSON in `.lazy-harness/fixtures/context-tier/context-tier-manifest.sample.json`.
...
Future phases may ingest the manifest into a deterministic record/source cache, but only after dogfooding and an explicit SDD/schema update.
...
- Primary files:
  ...
  - `.lazy-harness/fixtures/context-tier/context-tier-manifest.sample.json` — schema/sample fixture used by self-test.
