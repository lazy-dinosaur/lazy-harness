...
def check_operational_state_packet_phase6() -> None:
    """Phase 6 explicit operational-state packet stays advisory and manual-only."""
    sdd_path = LAZY / "spec" / "platform" / "operational-state-packet.md"
    tdd_path = LAZY / "tests" / "operational-state-packet.md"
    schema_path = LAZY / "schemas" / "operational-state-packet.schema.json"
    script_path = LAZY / "scripts" / "operational-state.ts"
    lazy_bin = LAZY / "bin" / "lazy"

    for path in (sdd_path, tdd_path, schema_path, script_path):
        if not path.exists():
            fail(f"operational-state Phase 6 missing file: {path.relative_to(ROOT)}")
    ...
    for expected in (
        ...
        if expected not in sdd:
            fail("operational-state SDD missing invariant: " + expected)

    tdd = tdd_path.read_text(encoding="utf-8")
    for expected in (
        "Raw message string",
        "fallback-needed",
        "Lifecycle hooks do not call `operational-state`",
        "Layer completeness gate",
    ):
        if expected not in tdd:
            fail("operational-state TDD missing regression/impact note: " + expected)

    schema = json.loads(schema_path.read_text(encoding="utf-8"))
    if schema.get("title") != "OperationalStatePacket":
        fail("operational-state schema title mismatch")
    required = set(schema.get("required", []))
    expected_required = {"schemaVersion", "generatedAt", "source", "taskKind", "requiredReads", "recommendedReads", "capabilities", "evidence", "risk", "notes"}
    if required != expected_required:
        fail("operational-state schema required keys mismatch: " + json.dumps(sorted(required), ensure_ascii=False))
    task_enum = set(schema.get("definitions", {}).get("taskKind", {}).get("enum", []))
    if task_enum != {"unknown", "implementation", "planning", "validation", "recording"}:
        fail("operational-state schema taskKind enum mismatch")
    risk_enum = set(schema.get("definitions", {}).get("risk", {}).get("enum", []))
    if risk_enum != {"low", "medium", "high", "unknown"}:
        fail("operational-state schema risk enum mismatch")

    source = script_path.read_text(encoding="utf-8")
    for expected in ("export function buildOperationalState", "raw-message-omitted", "fallback-needed"):
        if expected not in source:
            fail("operational-state script missing expected phrase: " + expected)
    if "writeFileSync" in source or "runtimeStatePath" in source or "journal" in source.lower():
        fail("operational-state script must not write/journal runtime state in Phase 6")
    if "operational-state" not in lazy_bin.read_text(encoding="utf-8"):
        fail("lazy CLI missing operational-state dispatch")
    ...
    if completed.returncode != 0:
        fail("lazy operational-state JSON failed:\n" + completed.stdout + completed.stderr)
    if raw_message in completed.stdout or "raw-private-operational-state-message" in completed.stdout:
        fail("operational-state JSON output leaked raw message")
    packet = json.loads(completed.stdout)
    for key in expected_required:
        if key not in packet:
            fail("operational-state packet missing key: " + key)
    if packet.get("schemaVersion") != "1.0" or packet.get("source", {}).get("method") != "operational-state-v1":
        fail("operational-state packet schema/method mismatch: " + completed.stdout)
    if packet.get("taskKind") != "validation":
        fail("operational-state should classify validation message coarsely: " + completed.stdout)
    if packet.get("risk") != "medium":
        fail("operational-state validation task should be medium risk: " + completed.stdout)
    if packet.get("source", {}).get("fallbackNeeded") is not True:
        fail("operational-state missing-index packet should set fallbackNeeded=true")
    notes_text = "\n".join(packet.get("notes", []))
    if "fallback-needed" not in notes_text or "raw-message-omitted" not in notes_text:
        fail("operational-state packet missing fallback/raw-message notes: " + completed.stdout)
    required_reads = packet.get("requiredReads", [])
    if not any(item.get("path") == ".lazy-harness/spec/platform/operational-state-packet.md" and item.get("reason") for item in required_reads):
    ...
    if offenders:
        fail("operational-state must not be wired into lifecycle hooks by default: " + json.dumps(offenders, ensure_ascii=False))

    print("✓ operational-state packet Phase 6 ok")
...
def main() -> None:
    ...
    checks: list[tuple[callable, str]] = [
        ...
        (check_operational_state_packet_phase6, "BOTH"),
