...
def check_jcode_wiring_repairs_markerless_bash_hook_default() -> None:
    ...
    try:
        ...
        if "Generated by lazy-harness" not in hook or "check-rule-action-boundary.py" not in hook:
            fail("markerless generated bash hook was not refreshed with rule boundary wiring:\n" + hook)
        archive = temp / ".jcode" / "archive"
        archived_names = {p.name for p in archive.iterdir()} if archive.exists() else set()
...
def check_rule_action_boundary_pr_body_guard() -> None:
    """Stored PR body records must bind to gh PR mutation boundaries."""
    ...
    try:
        ...
        if without_record.strip():
            fail("PR guard should stay silent without host PR format record:\n" + without_record)
        ...
        if "Rule binding violation" not in blocked or "## Why" not in blocked or "## What" not in blocked or "## Task" not in blocked:
            fail("PR guard did not block malformed body with required sections:\n" + blocked)
        ...
        if valid.strip():
            fail("PR guard should allow valid body:\n" + valid)
        ...
        if valid_file.strip():
            fail("PR guard should allow valid --body-file:\n" + valid_file)
    finally:
        shutil.rmtree(temp, ignore_errors=True)
...
def check_jcode_wiring_rule_action_boundary_hook() -> None:
    """Generated bash hook must call framework-owned rule action boundary helper."""
    source = (LAZY / "scripts" / "jcode-wiring.ts").read_text(encoding="utf-8")
    required = [
    ...
    missing = [phrase for phrase in required if phrase not in source]
    if missing:
        fail("jcode wiring missing rule action boundary hook phrases: " + json.dumps(missing, ensure_ascii=False))
    print("✓ jcode wiring rule action boundary hook ok")
