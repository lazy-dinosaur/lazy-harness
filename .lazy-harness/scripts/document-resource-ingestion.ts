#!/usr/bin/env bun
/**
 * Document Resource Ingestion — inspect mode (SDD: spec/platform/document-resource-ingestion.md)
 *
 * Slice 1 is read-only: scan root-bound non-harness documents, score trust
 * signals, cluster duplicates, and emit a reviewable report. It never writes
 * canonical DDD/SDD/BDD/TDD/ADR/SSOT records.
 */
...
function printHelp(): void {
  console.log(`Document Resource Ingestion — inspect mode\n\nUsage:\n  bun .lazy-harness/scripts/document-resource-ingestion.ts --mode inspect [--format md|json] [--root <path>] [--include <glob>] [--max-files N]\n\nInspect mode is read-only. It scans non-harness docs, scores freshness/authority/duplication/contamination, and prints a review report.`)
}
