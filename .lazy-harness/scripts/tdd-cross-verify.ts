...
function questionForMissingTest(file: string, now: string): TddQuestion {
  ...
  return {
    ...
    options: [
      {
        id: 'A',
        label: `${suggestedPath} 작성`,
        description: 'Recommended. 변경된 source 코드의 회귀 테스트를 이번 작업에 포함합니다.',
        effects: [{ kind: 'tdd-require-test', target: file, suggestedPath }],
      },
      {
        id: 'B',
        label: '의도적 refactor-only 로 기록',
        description: '동작 변경이 없음을 decision log 에 남기고 이번 gate 를 통과시킵니다.',
        effects: [{ kind: 'decision-log', summary: 'refactor-only without test change', reason: file }],
      },
      {
        id: 'C',
        label: '추후 작성으로 defer',
        description: '이번 범위 밖이지만 추후 테스트 필요성을 명시합니다.',
