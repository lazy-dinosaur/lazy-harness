...
const MANDATORY_LAYER2_BLOCK_START = '# BEGIN lazy-harness mandatory Layer 2 force-gates'
const MANDATORY_LAYER2_BLOCK_END = '# END lazy-harness mandatory Layer 2 force-gates'
...
function mandatoryLayer2HooksBlock(): string {
  return `${MANDATORY_LAYER2_BLOCK_START}
# Required by lazy-harness enforcement policy. Preserve local config above/below,
# but keep these hooks wired so record-first checks run before file mutation.
[[hooks.commands]]
...
blocking = true
timeout_ms = 3000
${MANDATORY_LAYER2_BLOCK_END}
`
}

function hasMandatoryLayer2Hook(existing: string, tool: 'edit' | 'write' | 'multiedit'): boolean {
  const hookPattern = new RegExp(
    String.raw`\[\[hooks\.commands\]\][\s\S]*?event\s*=\s*["']tool\.execute\.before["'][\s\S]*?tool\s*=\s*["']${tool}["'][\s\S]*?command\s*=\s*["']\.lazy-harness/hooks/lifecycle/on-tool-execute-before\.sh["'][\s\S]*?blocking\s*=\s*true`,
    'm'
  )
  return hookPattern.test(existing)
}

function ensureMandatoryLayer2Hooks(options: JcodeWiringOptions, path: string): void {
  if (!existsSync(path)) return
  const existing = readFileSync(path, 'utf8')
  if (
    hasMandatoryLayer2Hook(existing, 'edit') &&
    hasMandatoryLayer2Hook(existing, 'write') &&
    hasMandatoryLayer2Hook(existing, 'multiedit')
  ) {
    return
  }

  const stripped = existing.replace(
    new RegExp(`${MANDATORY_LAYER2_BLOCK_START}[\\s\\S]*?${MANDATORY_LAYER2_BLOCK_END}\\n?`, 'm'),
    ''
  ).trimEnd()
  const next = `${stripped}\n\n${mandatoryLayer2HooksBlock()}`
  if (options.dryRun) {
    log(options, `  [dry] patch mandatory Layer 2 hooks into ${path}`)
    return
  }
  writeFileSync(path, next)
  log(options, `  ✓ patched mandatory Layer 2 hooks into ${path}`)
}
...
export function installJcodeWiring(options: JcodeWiringOptions): void {
  ...
  ensureMandatoryLayer2Hooks(options, configPath)
