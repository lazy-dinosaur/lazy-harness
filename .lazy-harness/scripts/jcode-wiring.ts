...
const MESSAGE_RECEIVED_HOOK_START = '# BEGIN lazy-harness message.received direct-search hook'
const MESSAGE_RECEIVED_HOOK_END = '# END lazy-harness message.received direct-search hook'
...
function messageReceivedManagedRegex(): RegExp {
  return new RegExp(`(?:${MESSAGE_RECEIVED_HOOK_START}|${LEGACY_MESSAGE_RECEIVED_HOOK_START})[\\s\\S]*?(?:${MESSAGE_RECEIVED_HOOK_END}|${LEGACY_MESSAGE_RECEIVED_HOOK_END})\\n?`, 'm')
}
...
function ensureMessageReceivedHook(options: JcodeWiringOptions, path: string): void {
  ...
  if (existing.includes(MESSAGE_RECEIVED_HOOK_START) || existing.includes(LEGACY_MESSAGE_RECEIVED_HOOK_START)) {
    const next = existing.replace(managedRe, messageReceivedHookBlock()).trimEnd() + '\n'
    if (next === existing) return
    if (options.dryRun) {
      log(options, `  [dry] refresh message.received direct-search hook in ${path}`)
      return
    }
    writeFileSync(path, next)
    log(options, `  ✓ refreshed message.received direct-search hook in ${path}`)
    return
  }
  ...
  if (options.dryRun) {
    log(options, `  [dry] patch message.received direct-search hook into ${path}`)
    return
  }
  writeFileSync(path, next)
  log(options, `  ✓ patched message.received direct-search hook into ${path}`)
}
