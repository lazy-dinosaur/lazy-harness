...
const MESSAGE_RECEIVED_HOOK_START = '# BEGIN lazy-harness message.received context hook'
const MESSAGE_RECEIVED_HOOK_END = '# END lazy-harness message.received context hook'
...
function ensureMessageReceivedHook(options: JcodeWiringOptions, path: string): void {
  ...
  const stripped = existing.replace(
    new RegExp(`${MESSAGE_RECEIVED_HOOK_START}[\\s\\S]*?${MESSAGE_RECEIVED_HOOK_END}\\n?`, 'm'),
    ''
  ).trimEnd()
  const next = `${stripped}\n\n${messageReceivedHookBlock()}`
  if (options.dryRun) {
    log(options, `  [dry] patch message.received context hook into ${path}`)
    return
  }
  writeFileSync(path, next)
  log(options, `  ✓ patched message.received context hook into ${path}`)
}
...
function ensureReadDebtPermitHook(options: JcodeWiringOptions, path: string): void {
  ...
  const stripped = existing.replace(
    new RegExp(`${READ_DEBT_HOOK_START}[\\s\\S]*?${READ_DEBT_HOOK_END}\\n?`, 'm'),
    ''
  ).trimEnd()
  const next = `${stripped}\n\n${readDebtPermitHookBlock()}`
  if (options.dryRun) {
    log(options, `  [dry] patch read-debt action permit hook into ${path}`)
    return
  }
  writeFileSync(path, next)
  log(options, `  ✓ patched read-debt action permit hook into ${path}`)
}
