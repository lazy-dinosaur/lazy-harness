...
- `tool.execute.before` 기반 수동/fixture 검증은 남아 있지만, 보편 실시간 gate 로 가정하지 않는다. 판단이 복잡하면 `lazy route --message "..." --format=md --log` 로 workflow route 를 받아 기본 루트를 압축하고 telemetry 를 남긴다. Router 는 advisory 이며 record-first/default-unknown/option-gate/queue-close 를 약화하지 않는다 (ADR 0037).
