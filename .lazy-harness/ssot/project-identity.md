...
- Must:
  ...
  - interpret ambiguous evidence as lazy-harness agent/runtime behavior, not an unrelated product UI bug, unless redirected
...
When the user shows screenshots, transcripts, or short ambiguous references during this work, interpret them first as evidence about lazy-harness agent behavior and harness tuning, not as a request to fix an unrelated product UI, unless the user explicitly redirects the task.

## Scope

- Current host: `/home/lazydino/dev/lazy-harness`
- Framework product: lazy-harness
- Runtime support: Pi is stable primary; OMP is Experimental; active Jcode integration is decommissioned by ADR 0059.
...
- If a runtime-local skill or overlay from a downstream host is accidentally loaded while working in `/home/lazydino/dev/lazy-harness`, treat it as non-canonical for this source repo when it conflicts with this SSOT.
...
- Improve Pi/OMP runtime integration when the change belongs to the framework.
...
- Runtime-local transport and personal workflow overlays are non-canonical; shared rules belong in `.lazy-harness` records.
