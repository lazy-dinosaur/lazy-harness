...
- Applies when:
  ...
  - generating or inspecting `context-index.json` projectProfile metadata
...
- Must:
  ...
  - keep generated `context-index.json` optional and derived, never canonical
...
- Implementation hints:
  - Files: `.lazy-harness/project/feature-navigation.xml`, `.lazy-harness/scripts/context-index.ts`, `.lazy-harness/scripts/self-test.py`
...
- Derived cache: `.lazy-harness/generated/context-index.json` when explicitly generated
...
- Primary files:
  ...
  - `.lazy-harness/knowledge/graph.jsonl` — graph rows linking this SSOT, the source map, context-index implementation, and self-test protection.
- Key symbols:
  - `parseFeatureNavigation` (`context-index.ts`) — reads `.lazy-harness/project/feature-navigation.xml`.
  - `mergeProjectProfile` (`context-index.ts`) — merges feature aliases/routes/components/files/tests into referenced record entries.
  - `buildContextIndex` (`context-index.ts`) — emits the derived context index with `projectProfile` metadata.
  - `check_source_feature_navigation_phase3` (`self-test.py`) — verifies this source map remains complete enough for Phase 3.
- Flow:
  1. Agent or CLI builds context index.
  2. `context-index.ts` parses source feature navigation if present.
