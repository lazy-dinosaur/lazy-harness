...
- **Pi-first runtime**: Pi is the stable primary runtime; OMP uses the same package core as an Experimental integration.
...
└── .lazy-harness/            # Canonical framework body, source of truth
    ...
    ├── decisions/            # ADRs and retained decision history
    ├── domain/               # DDD records
...
After framework installation, `lazy init` or `lazy agent activate` creates private project-local Pi/OMP activation pointers and Pi skill settings. Pi is the stable primary runtime; OMP remains Experimental. Jcode integration is decommissioned by ADR 0059.
...
For day-to-day use, this is the command to remember. It refreshes the cached source checkout, syncs `.lazy-harness/`, and checks or repairs Pi/OMP activation while preserving host memory.
...
If `.lazy-harness/` exists but Pi/OMP activation pointers or shared package surfaces are missing, run:
```bash
cd /path/to/host
...
Pi is the stable primary Lazy-Harness runtime. OMP remains Experimental through the shared `packages/lazy-harness-pi` core and separate wrapper UX. Active Jcode integration was removed by ADR 0059; historical records remain available to explain the earlier adapter work and the runtime-neutral improvements retained from it.
