...
5. Validate proportionally: use `lazy check` during edit loops, focused/affected checks for changed behavior, and one `lazy validate --plan standard` only after the final mutation. Reserve direct `lazy test` for an explicit fresh full-regression request or commit/push/release gate; do not rerun it merely because an evidence capsule changed. Do not run product-wide typecheck/lint/build just to "cover all bases"; only do that when the user asked, the host test-strategy record requires it, or product code changed.
