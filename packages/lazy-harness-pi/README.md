...
- The package exposes one dedicated `lazy-harness.record-reader` Pi Subagents role. It performs map-first canonical `.lazy-harness` record loading only; the Parent keeps source/tests, decisions, mutation, validation, and the final answer.
- For a Reader-managed work unit, launch the Reader fresh/read-only with explicit `openai-codex/gpt-5.6-luna:low`, Agent Contract v1, `acceptance:false`, `output:false`, and `artifacts:false` unless a separate task-specific model override was approved. Its task uses exact `root:`, `revision:`, `model:`, `evidenceEpoch:`, `maxReadCalls:`, `maxRequestedLines:`, derived `maxLinesPerRead`, and `task:` lines; task model must equal launch model. Continue the Parent source/test lane with native/simple read-only tools, wait for actual Reader content, then call `lazy_reader_join` with status/result marker and revision/evidence epoch. Normally omit run ID, counters and `recordPaths`; trusted runtime state supplies them, including allowed/used budgets, per-read cap/max observed limit and failed-call count. The full successful-read path set is revalidated and cached even if a legacy caller reports only a subset; unread or duplicate supplied paths still fail. A process-complete acknowledgement alone is not a join.
- A matching `complete` join satisfies the transitional search/read-debt bridge without duplicate Parent record reads. Missing/incomplete/conflict/stale/budget-exceeded Reader results use bounded direct Parent fallback.
- In non-interactive Pi text/print mode, the substantive Parent response remains primary stdout; any post-response advisory is emitted to stderr and does not queue a second assistant turn that could replace it.
